...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    if (!process.env.NEXT_PUBLIC_BASE_URL_LOCAL) {
      console.warn('NEXT_PUBLIC_BASE_URL_LOCAL not found, using fallback metadata');
      return {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL_LOCAL}/posts?slug=${slug}&_embed&_fields=id,title,excerpt,slug,date,meta,rankmath_head,_embedded`;
    ...
    if (!response.ok) {
      ...
      return {
        title: `${slug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} - BootPrices`,
        description: 'Read the latest boot reviews and guides on BootPrices.',
      };
    }
    ...
    const description = post.meta?.rank_math_description || stripHtml(post.excerpt.rendered).slice(0, 160);
    const canonical = post.meta?.rank_math_canonical_url || `${process.env.NEXT_PUBLIC_SITE_URL}/blog/${post.slug}`;
    const ogImage = post.meta?.rank_math_facebook_image || post._embedded?.["wp:featuredmedia"]?.[0]?.source_url;
    ...
    return {
      ...
      twitter: {
        ...
        images: post.meta?.rank_math_twitter_image || ogImage ? [post.meta?.rank_math_twitter_image || ogImage] : undefined,
        site: '@BootPrices',
        creator: '@BootPrices',
      },
      robots: {
    ...
    return {
      ...
      openGraph: {
        title: fallbackTitle,
        description: 'Read the latest boot reviews and guides on BootPrices.',
        url: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://bootprices.com'}/blog/${slug}`,
        siteName: 'BootPrices',
        type: 'article',
      },
      twitter: {
